...
public class LevelServiceImpl implements LevelService {
    ...
    public PageData findByid(BigInteger id) throws Exception {
        return (PageData) dao.findForObject("LevelMapper.selectByPrimaryKey", id);
    }

    @Override
    public boolean update(PageData pd) throws Exception {
        int a = (int) dao.update("LevelMapper.updateByPrimaryKey", pd);
        if(a == 1){
            return true;
        }else{
            return false;
    ...
    public boolean delete(PageData pd) throws Exception {
        ...
        int a = (int) dao.update("LevelMapper.deleteByPrimaryKey", pd);
        if(a == 1){
            return true;
        }else{
            return false;
